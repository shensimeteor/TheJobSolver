...
class textProcess:
    ...
    def query_type_wordcloud(self):

            collection = self.client[self.name_database][self.name_collection]
            query_list = collection.distinct("query_keywords")
            for q in query_list:
                l = []
                for post in collection.find({'query_keywords':q}): #iterate through each document
                   #pprint.pprint(post['query_jobtyp_explvl'])
                ...
                #print(l)
                self.generate_word_cloud(l,q) #generate word cloud for every query type
                print("done for ---",q)
    def generate_word_cloud(self,l,q):
